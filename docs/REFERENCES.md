...
- `src/prompts/{analyze,change,check,cover,create,fix,implement,new,readme,recreate,refactor,references,renumber,workflow,write}.md`: Terminal output literals now require a trailing bell marker `\x07` for all fixed completion, warning, and error output strings (including final STATUS lines and exact-string early exits).
...
- `docs/REQUIREMENTS.md`: Updated `DES-002`, `REQ-023`, and `REQ-025` to require terminal exact-string outputs to end with `\x07` across completion, warning, and error termination paths.
