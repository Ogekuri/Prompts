...
- `docs/REQUIREMENTS.md`: Updated `REQ-019` to replace worktree-exit instruction `req --git-wt-exit` with explicit `cd <BASE_PATH>` and conditional `<GIT_PATH>`/`<BASE_PATH>` derivation.
