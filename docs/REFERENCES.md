...
- `src/prompts/{change,cover,fix,implement,new,recreate,refactor,references,readme,renumber,workflow}.md`: Step-order guard now enforces `req --git-wt-delete <WORKTREE_NAME>` only after `req --git-wt-create <WORKTREE_NAME>`; pre-create branches (including Git/docs checks) do not use delete instructions.
- `src/prompts/{change,cover,fix,implement,new,recreate,refactor,references,readme,renumber,workflow}.md`: Worktree-create failure branches (`ERROR: Worktree generation failed!`) no longer instruct `req --git-wt-delete <WORKTREE_NAME>` because `req --git-wt-create` failure already preserves clean git state.
...
- `docs/REQUIREMENTS.md`: Updated `REQ-019` and `REQ-025` to forbid `req --git-wt-delete <WORKTREE_NAME>` in branches before the create step and in prompts that never execute `req --git-wt-create <WORKTREE_NAME>`.
- `docs/REQUIREMENTS.md`: Updated `REQ-019` and `REQ-025` to forbid `req --git-wt-delete <WORKTREE_NAME>` in the create-failure interruption branch of `req --git-wt-create <WORKTREE_NAME>`.
