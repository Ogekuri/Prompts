...
- `src/prompts/{change,new,fix,cover,implement,refactor,check}.md`: Verification flow now uses requirements evidence plus `req --here --static-check` and removes unit/integration test-suite execution requirements.
...
- `docs/REQUIREMENTS.md`: Updated implementation-workflow verification requirements to remove mandatory pytest/unit-test-suite execution and require static-analysis-based verification.
...
- `docs/WORKFLOW.md`: Updated `PROC:main` lifecycle behavior to encode static-analysis-based verification, no-source static-check success handling, and non-execution of unit/integration test suites.
