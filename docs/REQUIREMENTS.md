...
- **REQ-019**: MUST enforce canonical phrasing for shared operational instructions, including project-operation preference for `req` commands, `req --git-check`, `req --docs-check`, `req --base-path`, `req --git-path`, `req --git-wt-name`, `req --git-wt-create <WORKTREE_NAME>`, `req --git-wt-exit`, `req --git-wt-delete <WORKTREE_NAME>`, and the Source Code Analysis Toolkit four-pillar workflow ordering 1→2→3→4; worktree-generation instructions MUST explicitly require deriving `<BASE_PATH>` with `req --base-path` and `<GIT_PATH>` with `req --git-path` before generating `<WORKTREE_NAME>` with `req --git-wt-name`; instructions that execute `req --git-wt-create <WORKTREE_NAME>` MUST explicitly require `cd ../<WORKTREE_NAME>` immediately after `req --git-wt-create <WORKTREE_NAME>` and before the next step; final repository-cleanliness verification MUST use `req --git-check` and MUST NOT use `git status --porcelain`; after successful worktree creation, every early-termination branch before merge-phase cleanup MUST instruct only `req --git-wt-delete <WORKTREE_NAME>` for cleanup before termination and MUST NOT instruct `git restore .`, `git checkout .`, or `git clean -fd`; branches before the create step MUST NOT instruct `req --git-wt-delete <WORKTREE_NAME>`; and the create-failure branch for `req --git-wt-create <WORKTREE_NAME>` MUST NOT instruct `req --git-wt-delete <WORKTREE_NAME>`.
