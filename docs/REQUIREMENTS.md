...
- **FIX-CTX-002**: MUST define the `## Scope` section to instruct: In scope: reproduce/triage defects with concrete evidence, implement the smallest safe fix under %%SRC_PATHS%%, verify with requirement evidence plus static analysis and conditional execution of existing unit tests using language-specific test-suite priority policy, then update `%%DOC_PATH%%/WORKFLOW.md` and `%%DOC_PATH%%/REFERENCES.md`, and commit; Out of scope: editing requirements, adding new features, or unnecessary refactors.
...
- **FIX-STP-005**: MUST define Step 5 to instruct: Generate Verification Delta by auditing ALL requirements with progressive-disclosure evidence (OK pointer-only, FAIL full), verifying defect resolution with requirement evidence plus `req --here --static-check`, running existing unit tests with language-specific priority policy, and implementing needed bug fixes.
