...
- **REQ-019**: MUST enforce canonical phrasing for shared operational instructions, including `<EXECUTION_ID>` generation via `date +"%Y%m%d%H%M%S"`, conditional `.req/config.json` copy during worktree setup, conditional removal of `../useReq-<PROJECT_NAME>-<ORIGINAL_BRANCH>-<EXECUTION_ID>/.req/config.json` before merge when `.gitignore` excludes `.req/config.json`, successful-merge cleanup that removes both the worktree and its branch, mandatory pre-step verification that both deletions succeeded, and Source Code Analysis Toolkit guidance defining a sequential four-pillar analysis pipeline (1: `%%DOC_PATH%%/WORKFLOW.md` runtime model for execution-flow comprehension, 2: `%%DOC_PATH%%/REFERENCES.md` symbol index with Doxygen-style metadata for target identification, 3: `req --find`/`req --files-find` code extraction where `--find` uses implicit `--here` and forbids `--base` with line-number prefixes `<n>:`, 4: `rg`/`git grep` for supplementary body-content search) and a recommended workflow ordering pillars 1→2→3→4.
